    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
      ...
      <div className="bg-[#0f172a] text-white py-16 no-print">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap gap-3 justify-center text-sm">
            <span className="bg-white/10 rounded-full px-4 py-2">$5,000 Onboarding</span>
            <span className="bg-white/10 rounded-full px-4 py-2">$500 / Month</span>
            <span className="bg-white/10 rounded-full px-4 py-2">5% Success Fee</span>
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm no-print">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3 flex-wrap">
          <span className="text-sm font-semibold text-gray-700 mr-2">Partnership Agreement — 5 / 500 Model</span>
      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="fade-up mb-16 bg-gradient-to-r from-indigo-900 to-purple-900 rounded-2xl p-8 text-white">
          <div className="grid md:grid-cols-3 gap-6 text-center">
            <div>
              ...
              <div className="text-sm text-indigo-200">Average recovered per label in Year 1</div>
            </div>
            <div>
              <div className="text-3xl font-black text-yellow-400 mb-1">48 hrs</div>
              <div className="text-sm text-indigo-200">Average time to first audit report</div>
            </div>
            <div>
              <div className="text-3xl font-black text-pink-400 mb-1">50%</div>
              <div className="text-sm text-indigo-200">Reduction in dispute resolution time</div>
            </div>
          </div>
        </div>
        ...
        <div className="fade-up mb-16">
          <h2 className="text-2xl font-bold text-gray-900 mb-8 text-center">Onboarding Timeline</h2>
            ...
            <div className="space-y-4">
              {TIMELINE.map((t, i) => (
                <div key={i} className="relative flex gap-6 items-start">
                  <div className="flex-shrink-0 w-16 h-16 bg-white border-2 border-indigo-200 rounded-2xl flex flex-col items-center justify-center shadow-md z-10">
                    <span className="text-xl">{t.icon}</span>
                    <span className="text-xs font-bold text-indigo-600">{t.day}</span>
                  </div>
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-md p-5 flex-1">
        ...
        <div className="fade-up mb-12">
          ...
          <div className="space-y-3">
            {SECTIONS.map(s => {
              ...
              return (
                <div key={s.num} className={`bg-white rounded-2xl shadow-md border overflow-hidden transition-all duration-200 ${isOpen ? 'border-indigo-200' : 'border-gray-100'}`}>
                  <button
                    onClick={() => toggle(s.num)}
                    className="w-full flex items-center gap-4 px-6 py-4 text-left hover:bg-gray-50 transition-colors"
                  >
                    <span className="flex-shrink-0 w-8 h-8 bg-indigo-100 text-indigo-700 rounded-lg flex items-center justify-center text-sm font-black">
                      {s.num}
                    </span>
                    <span className="font-semibold text-gray-900 flex-1">{s.title}</span>
                    <span className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center font-bold text-lg transition-all duration-300 ${isOpen ? 'bg-indigo-600 text-white rotate-45' : 'bg-gray-100 text-gray-400'}`}>
                      +
                    </span>
                  </button>
                  <div className={`section-body ${isOpen ? 'open' : 'closed'} ${openSection === 'all' ? 'print-full' : ''}`}>
                    <div className="px-6 pb-6 border-t border-gray-100 pt-4">
                      {s.body.split('\n\n').map((para, pi) => (
                        <p key={pi} className="text-gray-600 text-sm leading-relaxed mb-3 last:mb-0">{para}</p>
                      ))}
                    </div>
                  </div>
                </div>
        ...
        {/* Signature block */}
        <div className="fade-up mb-12">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">Signature Page</h2>
          <div className="grid md:grid-cols-2 gap-8">
            {/* Provider */}
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
              <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Service Provider</div>
              <div className="text-xl font-black text-gray-900 mb-1">TrapRoyaltiesPro</div>
              <div className="text-sm text-gray-500 mb-8">support@traproyaltiespro.com</div>
              <div className="border-t-2 border-gray-300 pt-4 mb-2">
                <div className="h-12 flex items-end">
                  <span className="text-gray-300 text-sm italic">Authorized Signature</span>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-xs text-gray-400 mb-1">Printed Name</div>
            ...
            {/* Client */}
            <div className="bg-white rounded-2xl shadow-xl border border-indigo-200 p-8">
              <div className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-4">Partner Label (Client)</div>
              <div className="text-xl font-black text-gray-900 mb-1">_________________________</div>
              <div className="text-sm text-gray-500 mb-8">Label Name / Company</div>
              ...
              <div className="mt-4">
                <div className="text-xs text-gray-400 mb-1">Title / Position</div>
                <div className="border-b border-gray-200 h-8" />
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 text-center">
            ...
            >
              ...
              <span className="text-sm font-normal bg-white/20 rounded-full px-3 py-1">SHA-256 verified</span>
            </Link>
            <p className="text-gray-400 text-sm mt-3">Blockchain timestamped · Court-admissible · E-SIGN Act compliant</p>
          </div>
        </div>
        ...
        <div className="fade-up grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={f.label} className="bg-white rounded-xl p-4 shadow-md border border-gray-100 text-center">
              <div className="text-2xl mb-1">{f.icon}</div>
              <div className="text-xs text-gray-400 uppercase tracking-wide mb-1">{f.label}</div>
