        ...
        <div className="flex-1 p-8">
          {showBar && (
            <div className="mb-6 flex items-center gap-4 p-4 bg-indigo-50 rounded-xl border border-indigo-100">
              ...
              <select value={selectedMatter} onChange={(e) => { setSelectedMatter(Number(e.target.value)); setScanComplete(false); }}
                className="flex-1 px-4 py-2 border border-indigo-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
                {MATTERS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
              </select>
              <span className={`px-3 py-1 rounded-full text-xs font-bold ${matter.status === 'Urgent' ? 'bg-red-100 text-red-800' : matter.status === 'In Progress' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>
                {matter.status}
              </span>
            </div>
          )}
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                  ...
                  <form onSubmit={handleHandshakeSubmit} className="space-y-4">
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Track / Project Name</label>
                      <input type="text" value={handshakeTrack} onChange={e => setHandshakeTrack(e.target.value)} placeholder="e.g. Neon Dreams (Rough Mix v3)" className="w-full px-4 py-3 border rounded-lg text-black bg-white" required />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">ISRC / ISWC (optional)</label>
                      <input type="text" value={handshakeISRC} onChange={e => setHandshakeISRC(e.target.value)} placeholder="e.g. USUM72212345" className="w-full px-4 py-3 border rounded-lg text-black bg-white" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Rights Type</label>
                      <select value={handshakeRightsType} onChange={e => setHandshakeRightsType(e.target.value)} className="w-full px-4 py-3 border rounded-lg text-black bg-white">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Revenue Basis</label>
                      <select value={handshakeRevenueBasis} onChange={e => setHandshakeRevenueBasis(e.target.value)} className="w-full px-4 py-3 border rounded-lg text-black bg-white">
                    ...
                    <div>
                      ...
                      <select value={handshakeJurisdiction} onChange={e => setHandshakeJurisdiction(e.target.value)} className="w-full px-4 py-3 border rounded-lg text-black bg-white">
                    ...
                    <div className="border-t pt-4">
                      ...
                      <select value={handshakeRole} onChange={e => setHandshakeRole(e.target.value)} className="w-full px-3 py-2 border rounded-lg text-black bg-white text-sm">
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  ...
                  <div className="bg-white rounded-xl border border-indigo-200 p-6 space-y-4">
                    <div className="flex justify-between items-center border-b pb-3">
                      ...
                      <span className="px-2 py-1 bg-indigo-100 text-indigo-800 text-xs rounded-full font-bold">PENDING</span>
                    </div>
                    ...
                    {handshakeName && (
                      <div className="border-t pt-3">
                        ...
                        <div className="flex justify-between items-center p-2 bg-yellow-50 rounded-lg border border-yellow-200">
                          ...
                          <span className="text-sm font-bold text-yellow-700">{handshakePercentage || 0}% - PENDING</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                ...
                <div className="grid md:grid-cols-2 gap-4 mb-8">
                  {["Streaming Royalties","Sync Licensing","Performance Rights","360 Deal (All Revenue)"].map((s,i) => (
                    <label key={s} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-indigo-50">
                      <input type="checkbox" className="h-4 w-4 text-indigo-600 rounded" defaultChecked={i===3}/><span>{s}</span>
                    </label>
                  ))}
                </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center mb-8 hover:border-indigo-500 transition">
                  <p className="text-gray-600 mb-1">Drop catalog file here or <span className="text-indigo-600 font-medium cursor-pointer">browse</span></p>
                  <p className="text-sm text-gray-500">CSV, Excel, PDF up to 50MB</p>
                </div>
                {!scanRunning && !scanComplete && (
                  <button onClick={runScan} className="w-full py-4 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition text-lg">
                ...
                {scanRunning && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-lg font-medium text-indigo-900">Running forensic scan...</p>
                ...
                {scanComplete && (
                  <div>
                    <div className="flex items-center gap-3 mb-6 p-4 bg-green-50 rounded-xl border border-green-200">
                      ...
                      <p className="font-bold text-green-800">Scan Complete - {matter.name}</p>
                    </div>
                    ...
                    <div className="space-y-3 mb-6">
                      {[{l:"ASCAP Registration",s:"Missing publisher registration",c:"red"},{l:"BMI PRO Status",s:"IPI mismatch on co-writer",c:"yellow"},{l:"SoundExchange",s:"No neighboring rights claim",c:"red"},{l:"PRS/SOCAN",s:"International registrations current",c:"green"},{l:"Split Verification",s:"Over-allocation detected (108%)",c:"red"}].map((item,i) => (
                        <div key={i} className={`flex justify-between p-3 rounded-lg ${item.c==='red'?'bg-red-50':item.c==='yellow'?'bg-yellow-50':'bg-green-50'}`}>
                          <span className="font-medium">{item.l}</span>
                          <span className={`font-bold ${item.c==='red'?'text-red-600':item.c==='yellow'?'text-yellow-600':'text-green-600'}`}>{item.s}</span>
                        </div>
                      ))}
                    </div>
                    <div className="grid md:grid-cols-2 gap-4">
                      <button onClick={() => setActiveSection('generate-court-report')} className="py-3 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition">Generate Court Report</button>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <div className="grid md:grid-cols-2 gap-8">
                  <div>
                    ...
                    <div className="space-y-3 mb-6">
                      {["Ownership Breakdown","Split Inconsistencies","Registration Status (PROs)","Black Box Leakage","ISRC Verification","Audit Trail"].map((s,i) => (
                        <label key={s} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-indigo-50">
                          <input type="checkbox" className="h-4 w-4 text-indigo-600" defaultChecked={i<4}/><span>{s}</span>
                        </label>
                      ))}
                    </div>
                    <button onClick={() => handleDownload('court','Court-Ready Audit Report')} disabled={downloading==='court'} className="w-full py-4 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition disabled:opacity-50">
                  ...
                  <div className="bg-gray-50 rounded-xl p-6">
                    ...
                    <p className="text-green-600 text-sm mt-1">Hash Verified</p>
                    <p className="text-sm mt-1">Unclaimed: <strong className="text-green-600">{matter.amount}</strong></p>
                    <p className="text-sm">Leakage: <strong className="text-red-600">{matter.leakage}</strong></p>
                    <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 mt-4">
                      <QRCode value={`https://traproyaltiespro.com/verify/TRP-COURT-${selectedMatter}`} size={80}/>
                      <div><p className="text-xs font-bold text-green-700">QR Verification Seal</p><p className="text-xs text-gray-500">Included on all exports</p></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-gray-50 rounded-xl p-6">
                    ...
                    <p className="text-sm text-gray-600 mt-2">We demand payment of <strong>{matter.amount}</strong> within 30 days.</p>
                    <p className="text-sm text-gray-600 mt-2">Leron Rogers, Esq. - Fox Rothschild LLP</p>
                    <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 mt-4">
                      <QRCode value={`https://traproyaltiespro.com/verify/TRP-DEMAND-${selectedMatter}`} size={80}/>
                      <div><p className="text-xs font-bold text-green-700">QR Verification Seal</p><p className="text-xs text-gray-500">Included on export</p></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 max-w-3xl">
                <div className="bg-gray-50 rounded-xl p-6 mb-6">
                  ...
                  <p className="text-sm text-gray-600 mt-2">1. I am counsel for {matter.name}...</p>
                  <p className="text-sm text-gray-600">2. Unpaid royalties of {matter.amount} verified...</p>
                  <p className="text-sm text-gray-600">3. {matter.issues} material discrepancies identified...</p>
                  <p className="text-sm text-gray-600 mt-4">___________________________<br/>Notary Public</p>
                </div>
                <div className="flex items-center gap-4 p-4 bg-green-50 rounded-xl border border-green-200 mb-6">
                  ...
                  <div><p className="font-bold text-green-800">QR Verification Seal</p><p className="text-sm text-gray-600">Printed on exported affidavit</p></div>
                </div>
                <button onClick={() => handleDownload('affidavit','Affidavit of Counsel')} disabled={downloading==='affidavit'} className="w-full py-4 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition disabled:opacity-50">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {["Ownership","Splits","PRO Status","ISRC Data","Black Box","Audit Trail"].map((o,i) => (
                        <label key={o} className="flex items-center space-x-2 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-indigo-50">
                          <input type="checkbox" className="h-4 w-4 text-indigo-600" defaultChecked={i<4}/><span className="text-sm">{o}</span>
                        </label>
                      ))}
                    </div>
                    <button onClick={() => handleDownload('custom','Custom Audit Report')} disabled={downloading==='custom'} className="w-full py-4 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition disabled:opacity-50">
          ...
          {activeSection === 'export-hash-seal' && (
            <div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center max-w-2xl mx-auto">
                ...
                <div className="bg-gray-50 rounded-xl p-6 mb-6">
                  <p className="font-bold text-indigo-900 mb-3">Hash Seal Verification</p>
                ...
                <div className="flex gap-4">
                  <button onClick={() => handleDownload('court','Hash-Sealed Audit Report')} disabled={downloading==='court'} className="flex-1 py-4 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition disabled:opacity-50">
                    {downloading==='court'?'Generating...':' Download Sealed PDF'}
                  </button>
                  <button className="flex-1 py-4 border border-gray-200 rounded-lg font-medium hover:bg-gray-50 transition">Share Secure Link</button>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                <div className="flex justify-between items-start mb-8">
                  <div>
                    ...
                    <p className="text-green-600 font-medium mt-2">Hash Verified</p>
                  </div>
                  <div className="flex gap-3 items-center">
                    <QRCode value={`https://traproyaltiespro.com/verify/TRP-AUDIT-${selectedMatter}`} size={80}/>
                    <button onClick={() => handleDownload('court','Audit Report')} disabled={downloading==='court'} className="px-6 py-3 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition disabled:opacity-50">
                      {downloading==='court'?'...':' Download PDF'}
                    </button>
                  </div>
                </div>
                <table className="w-full">
                  <thead className="bg-gray-50"><tr><th className="p-3 text-left">Party</th><th className="p-3 text-left">Claimed</th><th className="p-3 text-left">Verified</th><th className="p-3 text-left">Status</th></tr></thead>
                  <tbody className="divide-y divide-gray-200">
                    <tr><td className="p-3">Artist (Primary)</td><td className="p-3">50%</td><td className="p-3 text-green-600">50%</td><td className="p-3 text-green-600">Verified</td></tr>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 max-w-3xl">
                <form className="space-y-6" onSubmit={e => { e.preventDefault(); setActiveSection('run-due-diligence'); }}>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div><label className="block text-sm font-medium text-gray-700 mb-2">Client Name</label><input type="text" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Artist / Estate Name"/></div>
                    <div><label className="block text-sm font-medium text-gray-700 mb-2">Matter Type</label><select className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"><option>Royalty Dispute</option><option>Catalog Due Diligence</option><option>360 Deal Audit</option><option>Pre-Release Verification</option></select></div>
                  </div>
          ...
          {activeSection === 'pre-release-verify' && (
            <div>
              ...
              <p className="text-gray-600 mb-6">Upload &rarr; Detect issues &rarr; Verify &rarr; Calculate payment &rarr; Download PDF</p>

              {/* Before/After */}
              <div className="grid grid-cols-2 gap-6 bg-white border border-gray-200 rounded-2xl p-5 mb-6 shadow-sm">
                <div className="border-r border-red-100 pr-6">
                  ...
                  <div className="flex items-center gap-2 flex-wrap text-sm">
                    ...
                    <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full">Issues</span>
                    <span className="text-gray-400">&rarr;</span>
                ...
                <div>
                  <h3 className="text-indigo-900 font-bold mb-2">With TrapRoyaltiesPro</h3>
                  <div className="flex items-center gap-2 flex-wrap text-sm">
                    <span className="bg-gray-100 px-3 py-1 rounded-full">Publisher</span>
                    <span className="text-gray-400">&rarr;</span>
                    <span className="bg-indigo-100 text-indigo-900 px-3 py-1 rounded-full border border-indigo-200">TRP Verified</span>
                    <span className="text-gray-400">&rarr;</span>
                    <span className="bg-gray-100 px-3 py-1 rounded-full">PRO</span>
                    <span className="text-gray-400">&rarr;</span>
                    <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full">Fast Payment</span>
                  </div>
                </div>
              </div>
                ...
                {['Upload Data','Issues Detected','Data Verified','Payment Ready'].map((label, i) => (
                  <div key={i} className="flex items-center">
                    <div className="flex flex-col items-center">
                      <div className={"w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm border-2 " + (splitStep > i || splitStep === i ? 'bg-indigo-900 border-indigo-900 text-white' : 'bg-white border-gray-300 text-gray-400')}>{i+1}</div>
                      <span className={"text-xs mt-1 " + (splitStep === i ? 'text-indigo-900 font-semibold' : 'text-gray-400')}>{label}</span>
                    </div>
                    {i < 3 && <div className={"w-12 h-1 mb-4 " + (splitStep > i ? 'bg-indigo-900' : 'bg-gray-200')}></div>}
                  </div>
                ))}
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition-all mb-4"
                    onClick={() => { const el = document.getElementById('prvPortalFile') as HTMLInputElement; if(el) el.click(); }}>
                    <p className="font-semibold text-gray-700 mb-1">Drop your split sheet here</p>
                  ...
                  <div className="flex justify-center gap-6 text-sm mb-4">
                    <button onClick={loadPerfectSplit} className="text-indigo-600 hover:underline font-medium">Load perfect sample</button>
                  ...
                  {splitErrors.length > 0 && (
                    <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
                      <p className="text-red-700 font-bold mb-2">{splitErrors.length} Issue{splitErrors.length > 1 ? 's' : ''} Detected</p>
                      <div className="text-sm text-red-600 mb-3 space-y-1">{splitErrors.map((e: string, i: number) => <p key={i}>- {e}</p>)}</div>
                      <button onClick={autoFixSplit} className="bg-indigo-900 text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-indigo-800">&#10024; Auto-Fix Issues</button>
                    </div>
                  )}
                  {splitData.length > 0 && (
                    <div className="bg-gray-50 rounded-xl p-4">
                      <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-200">
                        ...
                        <span className={"text-xs px-2 py-1 rounded-full font-bold " + (splitErrors.length === 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600')}>{splitErrors.length === 0 ? 'Ready' : splitErrors.length + " issues"}</span>
                      </div>
                      {splitData.map((item: any, i: number) => (
                        <div key={i} className={"flex justify-between items-center py-3 border-b border-gray-100 " + (!item.ipi || !item.name ? 'bg-red-50 -mx-4 px-4' : '')}>
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center text-indigo-900 font-bold text-sm">{(item.name||'?')[0]}</div>
                            <div>
                          ...
                          <span className={"font-bold text-sm " + (!item.ipi||!item.name ? 'text-red-600' : 'text-indigo-900')}>{item.percentage}%</span>
                        </div>
                      ))}
                      <div className="text-right text-sm text-gray-500 mt-2">Total: <span className="font-bold text-black">{splitData.reduce((s: number, i: any) => s+(i.percentage||0), 0).toFixed(1)}%</span></div>
                    </div>
                  )}
                  {splitData.length > 0 && splitErrors.length === 0 && splitStep < 2 && (
                    <button onClick={startSplitVerification} className="w-full mt-4 py-3 bg-indigo-900 text-white rounded-xl font-medium hover:bg-indigo-800 transition">Start Verification</button>
                  )}
                </div>
                  ...
                  {splitStep >= 2 && (
                    <div className="bg-gray-50 rounded-xl p-4 mb-4">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between"><span className="text-gray-500">Timestamp</span><span className="text-black">{splitTimestamp}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Status</span><span className="text-green-600 font-bold">Verified &#10003;</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Jurisdiction</span><span className="text-black">Georgia Law</span></div>
                      </div>
                      <div className="mt-3 p-2 bg-indigo-50 rounded-lg font-mono text-xs text-indigo-600 break-all">sha256: {splitVerifyId}...monad_sealed</div>
                    </div>
                  )}
                  {splitStep >= 2 && splitStep < 3 && (
                    <button onClick={() => setSplitStep(3)} className="w-full py-3 bg-indigo-900 text-white rounded-xl font-medium hover:bg-indigo-800 transition mb-4">Calculate Payment</button>
                  )}
                  {splitStep >= 3 && (
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 mb-4">
                      ...
                      <div className="flex gap-3 items-center">
                        <div className="relative flex-1">
                          ...
                          <input type="number" value={splitPayAmount} onChange={(e) => setSplitPayAmount(Number(e.target.value))} className="w-full pl-8 pr-4 py-3 border border-gray-200 rounded-full text-lg font-bold text-black bg-white focus:outline-none focus:border-indigo-400" />
                        </div>
                        <button onClick={() => setSplitStep(4)} className="bg-indigo-900 text-white px-5 py-3 rounded-full font-medium hover:bg-indigo-800">Calculate</button>
                      </div>
                  ...
                  {splitStep >= 4 && (
                    <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-4">
                      <div className="flex justify-between items-center mb-3">
                        <span className="font-bold text-green-800">Payment Summary</span>
                        <span className="text-2xl font-bold text-green-700">${splitPayAmount.toLocaleString()}</span>
                      </div>
                      <div className="bg-white rounded-lg p-3 mb-3 space-y-2 text-sm">
                        ...
                        <div className="flex justify-between border-t pt-2"><span className="font-bold text-black">Net Payment</span><span className="font-bold text-green-700">${(splitPayAmount*0.75).toLocaleString()}</span></div>
                      </div>
                      <div className="space-y-2">
                        {splitData.map((item: any, i: number) => {
                          const gross = splitPayAmount*(item.percentage/100);
                          return (
                            <div key={i} className="flex justify-between items-center py-2 border-b border-green-100">
                              <span className="text-sm text-black">{item.name} ({item.percentage}%)</span>
                              <div className="text-right">
                                <div className="font-bold text-green-700 text-sm">${gross.toLocaleString()}</div>
                                <div className="text-xs text-red-400">-${(gross*0.25).toLocaleString()} tax</div>
                                <div className="text-xs text-green-600 font-semibold">${(gross*0.75).toLocaleString()} net</div>
                              </div>
                            </div>
                          );
                        })}
                      </div>
                      <button onClick={downloadSplitReport} className="w-full mt-4 py-3 bg-indigo-900 text-white rounded-xl font-medium hover:bg-indigo-800 transition">&#128196; Download Payment Report</button>
                    </div>
                  )}
                  {splitStep >= 4 && (
                    <button onClick={resetSplitWorkflow} className="w-full py-3 border border-gray-200 text-gray-500 rounded-xl font-medium hover:border-indigo-300 hover:text-indigo-600 mt-2">&#8635; Start New Verification</button>
                  )}
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-[600px] flex flex-col">
                <div className="flex items-center space-x-4 mb-6 pb-4 border-b border-gray-200">
                  <div className="w-12 h-12 bg-indigo-900 rounded-full flex items-center justify-center text-white font-bold">M</div>
                  <div><p className="font-bold">Metro Boomin</p><p className="text-sm text-gray-500">Online - End-to-End Encrypted</p></div>
                </div>
                <div className="flex-1 overflow-y-auto space-y-4 mb-4">
                  <div className="flex justify-end"><div className="bg-indigo-900 text-white p-4 rounded-2xl max-w-lg"><p>Ready to send demand letter?</p></div></div>
                ...
                <div className="flex gap-4">
                  ...
                  <button className="px-6 py-3 bg-indigo-900 text-white rounded-lg font-medium hover:bg-indigo-800 transition">Send</button>
                </div>
              </div>
            </div>
          )}
