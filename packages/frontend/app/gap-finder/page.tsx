...
const DEMO_TRACKS: Record<string, ProbeResult & { _demo: true; _revenue: ReturnType<typeof calcDemoRevenue>; _source: string }> = {
  ...
  // ── John Kim / Rights Administrator Demo Tracks ─────────────────────────────────────

  // Track 1: The "Viral Orphan" — high streams, no Rights Administrator registry match
  USATL2300001: (() => {
    const rev = calcDemoRevenue(48_500_000, false, false);
    return {
      _demo: true, _source: 'YouTube / ListenBrainz Secondary Validation Node',
      _revenue: rev,
      isrc: 'USATL2300001',
      status: 'RED',
      ...
      gaps: [
        { type: 'LINKAGE_GAP',    severity: 'CRITICAL', message: 'Unmapped ISRC: Market Signal Detected (External) | Registry Status: Unclaimed. No corresponding Neighboring Rights claim at Rights Administrator — est. $14,000+ in Black Box revenue.' },
        { type: 'IDENTITY_GAP',   severity: 'CRITICAL', message: 'Public Data Gap Detected: Master Recording (ISRC) lacks verified association. High potential for Unclaimed Neighboring Rights via Rights Administrator.' },
        { type: 'ISWC_GAP',       severity: 'HIGH',     message: 'No ISWC linked — 100% of publishing royalties unroutable to performing rights organizations.' },
      ],
      estimated_loss: rev.unclaimed,
      details: {
        recording:    { title: 'Atlanta Heat (Viral Edit)', artist: 'TrapArchitect', source: 'YouTube / ListenBrainz' },
        work:         { id: 'demo', title: 'Atlanta Heat (Viral Edit)', iswc: null, writers: [], total_writers: 0, writers_with_ipi: 0 },
        listen_stats: { total_listens: 48_500_000, unique_listeners: 3_200_000 },
        artist_identifiers: { ipi: null, isni: null },
      },
    };
  })(),

  // Track 2: The "Split Disaster" — 107.5% total claims
  SEMSM2300002: (() => {
    const rev = calcDemoRevenue(12_000_000, false, true);
    return {
      _demo: true, _source: 'MusicBrainz + ListenBrainz',
      _revenue: rev,
      isrc: 'SEMSM2300002',
      ...
      gaps: [
        { type: 'PERCENTAGE_GAP', severity: 'CRITICAL', message: 'Registry Conflict: 107.5% Total Claims Detected — overlapping producer percentages. Proportional Rescale required before Rights Administrator distribution.' },
        { type: 'ISWC_GAP',       severity: 'HIGH',     message: 'No ISWC — publishing royalty chain broken across 3 conflicting contributor claims.' },
      ],
      estimated_loss: rev.unclaimed,
      details: {
        recording: { title: 'Midnight In Stockholm', artist: 'Multiple Contributors', source: 'MusicBrainz' },
        work: {
          id: 'demo', title: 'Midnight In Stockholm', iswc: null,
          writers: [
            { name: 'Producer A',  role: 'producer',  ipi: '00472915682' },
            { name: 'Producer B',  role: 'producer',  ipi: '00836125497' },
            { name: 'Co-Writer C', role: 'composer',  ipi: null },
          ],
          total_writers: 3, writers_with_ipi: 2,
        },
        listen_stats:       { total_listens: 12_000_000, unique_listeners: 1_200_000 },
        artist_identifiers: { ipi: null, isni: null },
      },
    };
  })(),

  // Track 3: The "Pseudonym Proxy" — identity resolution
  USCC23000003: (() => {
    const rev = calcDemoRevenue(22_000_000, true, true);
    return {
      _demo: true, _source: 'MusicBrainz + SMPT IPI Node (Sweden)',
      _revenue: { ...rev, leakage: '0.0', unclaimed: 0, min: 0, max: 0 },
      isrc: 'USCC23000003',
      status: 'GREEN',
      song_title: 'Code & Culture',
      artist: 'TrapArchitect',
      gaps: [],
      estimated_loss: 0,
      details: {
        recording: { title: 'Code & Culture', artist: 'TrapArchitect → Legal Entity: Leland Tyler Wayne', source: 'SMPT IPI Node (Sweden)' },
        work: {
          id: 'demo', title: 'Code & Culture', iswc: 'T-920.819.673-2',
          writers: [{ name: 'Leland Tyler Wayne (TrapArchitect)', role: 'composer', ipi: '0064010xxxx' }],
          total_writers: 1, writers_with_ipi: 1,
        },
        listen_stats:       { total_listens: 22_000_000, unique_listeners: 2_800_000 },
        artist_identifiers: { ipi: '0064010xxxx', isni: '0000000199887766' },
      },
    };
  })(),
};
...
export default function GapFinderPage() {
  ...
  // SMPT modal
  const [smptModal, setSmptModal] = useState<{ name: string; isrc: string; loss: number } | null>(null);
  ...
  const runSingleProbe = async (e: React.FormEvent) => {
    ...
    if (demoMode) {
      ...
      else setSingleError('No pre-indexed data for this ISRC in demo cache. Try: USATL2300001 (Atlanta Heat), SEMSM2300002 (Midnight In Stockholm), USCC23000003 (Code & Culture), USSM12200452 (Future), or USUM71703861 (Drake).');
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
              ...
              {demoMode && (
                <div className="mb-4 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      { label: 'Atlanta Heat (Viral Edit) — 48.5M streams', isrc: 'USATL2300001', badge: '🔴 UNCLAIMED' },
                      { label: 'Midnight In Stockholm — Split Conflict',     isrc: 'SEMSM2300002', badge: '🔴 107.5%' },
                      { label: 'Code & Culture — Identity Resolved',         isrc: 'USCC23000003', badge: '✅ CLEAN' },
            ...
            {singleResult && (
              <div className="bg-[#0f172a] border border-white/10 rounded-2xl overflow-hidden">
                ...
                <div className="flex items-center gap-4 p-5 border-b border-white/10">
                  ...
                  {/* SMPT SECURED badge */}
                  <button
                    onClick={() => setSmptModal({ name: singleResult.song_title || singleResult.isrc, isrc: singleResult.isrc, loss: singleResult.estimated_loss })}
                    className="flex items-center gap-2 bg-[#1e1b4b] border border-indigo-500 hover:border-violet-400 hover:shadow-[0_0_14px_rgba(139,92,246,0.35)] rounded-lg px-3 py-1.5 transition-all flex-shrink-0"
                  >
        ...
        <div className="grid grid-cols-3 gap-4 mt-10">
          {[
            ...
            { icon: '🟡', title: 'Identity Gap', desc: 'Artist has no IPI/ISNI — Rights Administrator neighboring rights claim will fail.' },
          ].map(f => (
      ...
      {/* SMPT Forensic Modal */}
      {smptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setSmptModal(null)}>
          <div className="bg-[#0d1117] border border-indigo-500/40 rounded-2xl p-6 max-w-md w-full mx-4 shadow-[0_0_40px_rgba(99,102,241,0.25)]" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center gap-3 mb-5">
              <span className="relative flex items-center justify-center w-10 h-10 border-2 border-indigo-500 rounded-[8px] rotate-[15deg] bg-[#1e1b4b]">
                <span className="text-green-400 text-lg -rotate-[15deg]">🔒</span>
                <span className="absolute -top-1.5 -right-1.5 w-3 h-3 bg-green-500 rounded-full border-2 border-[#0d1117] animate-pulse"></span>
              </span>
              <div>
                <p className="text-white font-black text-base tracking-wide">SMPT SECURED</p>
                <p className="text-indigo-400 text-xs">usesmpt.com · Forensic Verification Protocol</p>
              </div>
              <button onClick={() => setSmptModal(null)} className="ml-auto text-slate-500 hover:text-white text-xl leading-none">×</button>
            </div>
            {/* Track info */}
            <div className="bg-white/5 rounded-xl p-3 mb-4 text-xs">
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px] mb-1">Audited Track</p>
              <p className="text-white font-bold">{smptModal.name}</p>
              <p className="font-mono text-indigo-300 text-[10px]">{smptModal.isrc}</p>
            </div>
            {/* Checklist */}
            <div className="space-y-2 mb-5">
              {[
                { label: 'ISRC Indexed — MusicBrainz Node', pass: true },
                { label: 'ListenBrainz Activity Logged', pass: true },
                { label: 'Gap Analysis Complete', pass: true },
                { label: smptModal.loss > 0 ? `Revenue Leakage Flagged — $${smptModal.loss.toLocaleString()} Est.` : 'No Revenue Leakage Detected', pass: smptModal.loss === 0 },
                { label: 'Audit Timestamp Anchored (SHA-256)', pass: true },
                { label: 'Attorney-Grade Export Available', pass: true },
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-2.5 text-xs">
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-black ${item.pass ? 'bg-green-500/20 text-green-400 border border-green-500/40' : 'bg-red-500/20 text-red-400 border border-red-500/40'}`}>
                    {item.pass ? '✓' : '!'}
                  </span>
                  <span className={item.pass ? 'text-slate-300' : 'text-red-300'}>{item.label}</span>
                </div>
              ))}
            </div>
            {/* Footer */}
            <div className="flex items-center justify-between border-t border-white/10 pt-4">
              <p className="text-[9px] text-slate-600 uppercase tracking-widest">Powered by usesmpt.com</p>
              <a href="/free-audit" className="px-3 py-1.5 bg-indigo-700 hover:bg-indigo-600 text-white text-xs font-black rounded-lg transition">Full Audit →</a>
            </div>
          </div>
        </div>
      )}
